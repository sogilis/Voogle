			...
			if tt.giveTitle == "" || tt.giveEmptyBody || tt.giveFieldPart == "NOT-video" || tt.giveWrongMagic {
				...
				if tt.titleAlreadyExists {
					...
					mock.ExpectExec(createVideoQuery).
						WithArgs(VideoID, tt.giveTitle, contracts.Video_UPLOADING).
						WillReturnError(fmt.Errorf("Error while creating new video"))

					row := videosRows.AddRow(VideoID, tt.giveTitle, contracts.Video_UPLOADING, nil, t1, t1)
					...
					row := videosRows.AddRow(VideoID, tt.giveTitle, contracts.Video_FAIL_ENCODE, nil, t1, t1)
					mock.ExpectQuery(getVideoFromTitleQuery).WithArgs(tt.giveTitle).WillReturnRows(row)

					// Update video status : ENCODING
					mock.ExpectExec(updateVideoQuery).
						WithArgs(tt.giveTitle, contracts.Video_ENCODING, nil, t1, VideoID).
					...
					if tt.lastUploadFailed {
						...
						row := videosRows.AddRow(VideoID, tt.giveTitle, contracts.Video_FAIL_UPLOAD, nil, t1, t1)
						mock.ExpectQuery(getVideoFromTitleQuery).WithArgs(tt.giveTitle).WillReturnRows(row)
					...
					if tt.createUploadFail {
						...
						mock.ExpectExec(updateVideoQuery).
							WithArgs(tt.giveTitle, contracts.Video_FAIL_UPLOAD, nil, t1, VideoID).
							WillReturnResult(sqlmock.NewResult(0, 1))
						...
						mock.ExpectExec(updateVideoQuery).
							WithArgs(tt.giveTitle, contracts.Video_UPLOADED, AnyTime{}, t1, VideoID).
							WillReturnResult(sqlmock.NewResult(0, 1))
						...
						// Update video status : ENCODING
						mock.ExpectExec(updateVideoQuery).
							WithArgs(tt.giveTitle, contracts.Video_ENCODING, AnyTime{}, t1, VideoID).
							WillReturnResult(sqlmock.NewResult(0, 1))
					}
				}
			}
